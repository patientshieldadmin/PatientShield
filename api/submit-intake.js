export default async function handler(req, res) {
  ...
  try {
    ...
    let analysisFindings = [
      { category: 'Document Ingestion & Parsing', description: `Ingested ${fileName} for high-acuity forensic audit review.` }
    ];
    let disputeLetterDraft = `[HOSPITAL BILLING DISPUTE & ITEMIZED AUDIT REQUEST]\n\nDear Billing Compliance Department,\n\nPatient Name: ${actualPatientName}\nFacility: ${actualFacilityName}\n\nWaiting for forensic AI analysis completion...`;

    // Elite Forensic AI Audit Execution
            ...
            messages: [
              {
                ...
                1. "extractedTotal": The exact gross total bill or balance amount found on the document (formatted as currency).
                ...
                5. "estimatedSavings": The precise dollar amount of potential savings derived strictly from the sum of the specific line-item discrepancies and markup corrections identified in this audit. Base this entirely on what you find in the bill text, not a flat percentage shortcut.
                6. "disputeLetter": A comprehensive, professional, compliance-ready hospital dispute letter that explicitly incorporates the extracted patient name, extracted facility name, extracted total, and specific line-item findings with regulatory citations (e.g., No Surprises Act, NCCI bundling edits, Medicare fair-market cost benchmarks).
